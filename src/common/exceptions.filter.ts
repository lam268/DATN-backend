...
import { CUSTOM_ERROR_MESSAGE_PREFIX, HttpStatus } from './constants';
...
const translateErrorValidator = async (
    ...
    const errorMessages = await Promise.all(
        errors.map(async (error: ValidationErrorItem) => {
            ...
            // translate message
            let errorMessage = '';
            if (message.startsWith(CUSTOM_ERROR_MESSAGE_PREFIX)) {
                errorMessage = await i18n.translate(
                    message.slice(CUSTOM_ERROR_MESSAGE_PREFIX.length),
                );
            } else if (context.name) {
                errorMessage = await i18n.translate(context.name, {
                    args: context,
                });
            } else {
                errorMessage = await i18n.translate(key, { args: context });
            }
            return {
                key: path.join('.'),
